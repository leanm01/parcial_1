...
int utn_getInt(char* mensaje, char* mensajeError, int* pResultado,int reintentos,int maximo,int minimo)//funcion que valida si es un entero
{
	int retorno = -1;
	int bufferInt;//espacio en memoria
	int resultadoScanf;
...
int menu()
{
   ...
   // printf("                ABM                      \n\n");
	...
	printf("4) ALTA RECAUDACION\n");
	printf("5) REFINANCIAR y SALDADA\n");
	printf("6) MOSTRAR CONTRIBUYENTES\n");
	printf("7) MOSTRAR RECAUDACION\n");
    printf("8) SALIR\n");
    utn_getInt("\nIngrese opcion: ", "Ingrese una opcion entre 1 y 6", &opcion, 3, 8, 1);
